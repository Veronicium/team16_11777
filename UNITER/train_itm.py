...
from data import (TokenBucketSampler, TokenBucketSamplerForItm, ConcatDatasetWithLens,
                  PrefetchLoader, TxtTokLmdb, ImageLmdbGroup,
...
def build_dataloader(dataset, collate_fn, is_train, opts):
    batch_size = opts.train_batch_size if is_train else 128
...
def main(opts):
    ...
    for txt_path, img_path in zip(opts.train_txt_dbs, opts.train_img_dbs):
        ...
        #                                      opts.negative_size))
        train_datasets.append(ItmDataset(txt_db, img_db, opts.itm_neg_prob))
    ...
    val_dataset = ItmDataset(val_txt_db, val_img_db, opts.itm_neg_prob)
    val_dataloader = build_dataloader(val_dataset, itm_ot_collate,
                                      False, opts)
    # eval
    ...
    eval_dataset_val = ItmDataset(val_txt_db, val_img_db, opts.itm_neg_prob)
    eval_loader_val = build_dataloader(eval_dataset_val, itm_ot_collate,
                                       False, opts)
    ...
    #                                    opts.inf_minibatch_size)
    eval_dataset_test = ItmDataset(val_txt_db, val_img_db, opts.itm_neg_prob)
    eval_loader_test = build_dataloader(eval_dataset_test, itm_ot_collate,
                                        False, opts)
    ...
    while True:
        train_dataloader = build_dataloader(
            train_dataset, itm_ot_collate, True, opts)
        for step, batch in enumerate(train_dataloader):
            ...
            # forward pass
            n_examples += batch['input_ids'].size(0)
            losses = model(batch, compute_loss=True, mode='phrase')
            # OT
            # if False:
            if type(losses) is tuple:
                itm_loss, ot_loss = losses
                itm_loss = itm_loss.mean()
                # if ot_loss is not None:
                if False:
                    ot_pos, ot_neg = ot_loss # pos and neg dist
                    ...
                    if not math.isnan(ot_pos) and not math.isnan(ot_neg):
                        loss = itm_loss + opts.itm_ot_lambda * ot_loss
                    else:
                        loss = itm_loss * (1+opts.itm_ot_lambda)
            ...
            if (step + 1) % opts.gradient_accumulation_steps == 0:
                ...
                if global_step % 100 == 0:
                    ...
                    if ot_loss is not None:
                        LOGGER.info(f'itm loss: {itm_loss.item()}'f'\tot loss: {ot_loss.item()}')
                    else:
...
def validate(model, val_loader):
    ...
    for batch in val_loader:
        ...
        n_ex += 1
        pbar.update(1)
    recall_at_1 = recall_at_1 / n_ex
    recall_at_5 = recall_at_5 / n_ex
    recall_at_10 = recall_at_10 / n_ex
