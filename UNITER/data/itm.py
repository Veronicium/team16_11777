...
class ItmDataset(DetectFeatTxtTokDataset):
    ...
    def __getitem__(self, i):
        ...
        for chunk in doc.noun_chunks:
            phrase2word.append(list(range(chunk.start, chunk.end)))

        phrases = [[-1]*MAX_PHRASE_LEN for _ in range(MAX_PHRASE_NUM)] # MAX_PHRASE_NUM * MAX_PHRASE_LEN matrix
        word_id = -1
        for tid, tok in enumerate(example['toked_caption']):
            if tok[:4] != '@@##':
                word_id += 1
            for pid in range( min(len(phrase2word), MAX_PHRASE_NUM)):
                if word_id in phrase2word[pid]:
                    for i in range(MAX_PHRASE_LEN):
                        if phrases[pid][i] == -1:
                            phrases[pid][i] = tid
                            break

        return input_ids, img_feat, img_pos_feat, attn_masks, target, phrases
...
def itm_ot_collate(inputs):
    (input_ids, img_feats, img_pos_feats, attn_masks, targets, phrases
     ) = map(list, unzip(inputs))
    ...
    # OT inputs
    targets = torch.cat(targets, dim=0)
    phrases = torch.tensor(phrases, dtype=int)
    phrase_pad = phrases[:,:,0] == -1
